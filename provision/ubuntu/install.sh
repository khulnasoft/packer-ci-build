...
# Install nodejs and npm, needed for the khulnasoft rtd sphinx theme
...
sudo -H pip3 install -r https://raw.githubusercontent.com/khulnasoft/khulnasoft/master/Documentation/requirements.txt
...
# https://github.com/khulnasoft/image-tools/blob/master/images/llvm/checkout-llvm.sh.
docker run --rm --name khulnasoft-llvm -d quay.io/khulnasoft/khulnasoft-llvm:a8c542efc076b62ba683e7699c0013adb6955f0f sleep 1000
docker cp khulnasoft-llvm:/usr/local/bin/clang /usr/bin/clang
docker cp khulnasoft-llvm:/usr/local/bin/llc /usr/bin/llc
docker stop khulnasoft-llvm
...
[Service]
ExecStart=/usr/bin/etcd --name=khulnasoft --data-dir=/var/etcd/khulnasoft --advertise-client-urls=http://192.168.36.11:9732 --listen-client-urls=http://0.0.0.0:9732 --listen-peer-urls=http://0.0.0.0:9733
Restart=on-failure
RestartSec=5
...
# Install hubble
cd /tmp
wget "https://github.com/khulnasoft/hubble/releases/download/v${HUBBLE_VERSION}/hubble-linux-amd64.tar.gz"
wget "https://github.com/khulnasoft/hubble/releases/download/v${HUBBLE_VERSION}/hubble-linux-amd64.tar.gz.sha256sum"
sha256sum --check hubble-linux-amd64.tar.gz.sha256sum || exit 1
sudo tar -xf "hubble-linux-amd64.tar.gz" -C /usr/bin hubble
sudo bash -c "echo 'HUBBLE_SERVER=unix:///var/run/khulnasoft/hubble.sock' >> /etc/environment"

# Install Khulnasoft's CLI
cd /tmp/
curl -L --remote-name-all https://github.com/khulnasoft/khulnasoft-cli/releases/latest/download/khulnasoft-linux-${ARCH}.tar.gz{,.sha256sum}
sha256sum --check --strict khulnasoft-linux-${ARCH}.tar.gz.sha256sum
sudo tar xzfC khulnasoft-linux-${ARCH}.tar.gz /tmp
mv /tmp/khulnasoft /usr/local/bin/khulnasoft-cli
rm khulnasoft-linux-${ARCH}.tar.gz{,.sha256sum}
...
# https://github.com/khulnasoft/khulnasoft/issues/2750
...
# CoreDumps https://github.com/khulnasoft/khulnasoft/issues/3399
...
# Add an exception for the khulnasoft repo for the root user to fix the
# "fatal: unsafe repository ('/home/vagrant/go/src/github.com/khulnasoft/khulnasoft' is owned by someone else)"
# error condition when running `sudo make install`
sudo git config --global --add safe.directory /home/vagrant/go/src/github.com/khulnasoft/khulnasoft
